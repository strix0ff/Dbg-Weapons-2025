...
util.AddNetworkString("octoweapons.recoil")
util.AddNetworkString("octoweapons.zoom")
--[[
net.Receive("octoweapons.zoom", function(_, ply)
    local delta = net.ReadInt(8)

    if not IsValid(ply) then return end

    local wep = ply:GetActiveWeapon()
    if not IsValid(wep) then return end
    if not wep.SightZoomLevels or not istable(wep.SightZoomLevels) then return end
    if #wep.SightZoomLevels < 1 then return end

    if not string.StartWith(wep:GetClass(), "weapon_octo_sniper") then return end

    wep.currentZoomLevel = wep.currentZoomLevel or 1

    wep.currentZoomLevel = math.Clamp(wep.currentZoomLevel + delta, 1, #wep.SightZoomLevels)
end)
]]--
-- 1 = left, 2 = right
...
function SWEP:MuzzleFlashCustom()
    ...
    net.Start('octoweapons.muzzleanimation')
        net.WriteEntity(self)
...
function SWEP:ChangeFireMode(mode)
    ...
    if not modeNum or not self.FireModes[modeNum] then return end

    self:SetNWString('firemode', modeNum)

    local strMode = self.FireModes[modeNum]

    if strMode == 'semi' then
        self.Primary.Automatic = false
    elseif strMode == 'auto' then
        self.Primary.Automatic = true
    elseif strMode == 'single' then
        self.Primary.Automatic = false
    end
end
...
function SWEP:Initialize()
    ...
    if firemode ~= "" and self.ChangeFireMode then
        self:ChangeFireMode(firemode)
    else
        self:ChangeFireMode(self.Primary.Automatic and 2 or 1)
    end

    self:SetReady(false)
    self:SetNWBool('CanSetReady', true)
    self:SetNWBool('CanFire', true)
end

function SWEP:SetReady(b)

	if not self:GetNWBool('CanSetReady') then return end
...
hook.Add('octolib.canUseAnimation', 'dbg_weapons', function(ply, cat)
	...
	local wep = ply:GetActiveWeapon()
	if IsValid(wep) and wep:GetNWBool('IsReady') and (cat.stand or cat.freeze) then
		return false
	end
...
function SWEP:Think()
	if table.IsEmpty(curAmmo) then
		curAmmo = {self:Clip1(), self:Ammo1()}
	end

	if curAmmo[1] ~= self:Clip1() or curAmmo[2] ~= self:Ammo1() then self:SetNWBool('updateLook', true) end

	local ply = self.Owner
	if (not self:GetNWBool('dbgLook') and self.canLook) or self:GetNWBool('updateLook') then
		local text = '- В магазине: ' .. self:Clip1() .. '\n- В запасе: ' .. self:Ammo1() .. '\n- Режим огня: ' .. (self.Primary.Automatic and 'Автоматический' or 'Одиночный')
		if self.itemData and self.itemData.desc and self.itemData.desc ~= L.desc_weapon then
			text = text .. '\n- ' .. self.itemData.desc
		end

		-- self:SetNWBool('dbgLook', {
		-- 	name = self:GetPrintName(),
		-- 	desc = text,
		-- 	time = 2.5,
		-- 	checkLoader = 'gunInspect',
		-- 	bone = 'ValveBiped.Bip01_R_Hand',
		-- 	posRel = self.LookPos or Vector(0, 0, 0),
		-- })
		self:SetNWBool('updateLook', false)
	end
	local ply = self.Owner
	if ply:KeyDown(IN_ATTACK2) and not self:GetNWBool('IsReady') then
		...
		self:SetReady(true)
	elseif !ply:KeyDown(IN_ATTACK2) and self:GetNWBool('IsReady') then
		self:SetReady(false)
	end

	if self:GetNWBool('IsReady') then
		...
		if IsValid(tgt) and tgt:IsPlayer() and self.CanScare and tgt:Team() ~= TEAM_ADMIN then
			...
			tgt:SetNWInt('ScareState', math.min(tgt:GetNWInt('ScareState', 0) + (FrameTime() / 3) * (self.ScareMultiplier or 1), 1))
			if tgt.lastScare <= 0.6 and tgt:GetNWInt('ScareState', 0) > 0.6 then
...
function SWEP:Reload()
    ...
    timer.Simple(.2, function()
        ...
        self:EmitSound(self.ReloadSound, 50)
    end)

    self:SetNWBool('CanSetReady', false)
...
hook.Add('Think', 'dbg-scare', function(ply)
	...
	for i, ply in ipairs(player.GetAll()) do
		local scare = ply:GetNWInt('ScareState', 0)
		if scare <= ply.lastScare and scare ~= 0 then
			scare = math.max(scare - FrameTime() / 10, 0)
			ply:SetNWInt('ScareState', scare)

			if ply.lastScare > 0.6 and ply:GetNWInt('ScareState', 0) <= 0.6 then
				ply:StopSound('d1_trainstation.cryingloop')
...
hook.Add('PlayerDisconnected', 'dbg-scare', function(ply)
	...
	if ply:GetNWInt('ScareState', 0) > 0.6 then
		ply:Kill()
...
hook.Add('octochat.canExecute', 'dbg-scare', function(ply, cmd)
	...
	if ply:GetNWInt('ScareState', 0) > 0.6 and table.HasValue(disabledCmds, cmd) then
...
hook.Add('closelook.canZoom', 'dbg-weapons', function(ply)
	...
	if IsValid(weapon) and weapon.HasZoom and weapon:GetNWBool('IsReady') then
		return true
	end
end)
